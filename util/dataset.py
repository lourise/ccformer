...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        if night:
            night_data_list_path = fss_list_root + 'night_data_list_{}.txt'.format(split)
            night_sub_class_file_list_path = fss_list_root + 'night_sub_class_file_list_{}.txt'.format(split)
        # Write FSS Data
        ...
        if night:
            with open(night_data_list_path, 'r') as f:
                f_str = f.readlines()
            self.night_data_list = []
            for line in f_str:
                img, mask = line.split(' ')
                self.night_data_list.append((img, mask.strip()))

            with open(night_sub_class_file_list_path, 'r') as f:
                f_str = f.read()
            self.night_sub_class_file_list = eval(f_str)
    ...
    def __getitem__(self, index):
        label_class = []
        # if self.night:
        #     image_path, label_path = self.night_data_list[index]
        # else:
        ...
        # supp night query day
        # if self.night:
        #     file_class_chosen = self.night_sub_class_file_list[class_chosen]
        # else:
        ...
        for k in range(self.shot):  
            ...
            support_label, support_label_mask, x_local = transform_anns(support_label, self.ann_type)   # mask/bbox
            support_label[ignore_pix[0],ignore_pix[1]] = 255
            support_label_mask[ignore_pix[0],ignore_pix[1]] = 255
            if not self.use_coco:
                support_label_padding = np.zeros_like(support_label)
                support_label_padding[support_label==255] = 255
            else:
                support_label_padding = np.zeros_like(support_label)
            # crop the foreground box of the support image
            # h, w = support_label_mask.shape
            # support_image = support_image[x_local[1]:x_local[3], x_local[0]:x_local[2]]
            # support_label = support_label[x_local[1]:x_local[3], x_local[0]:x_local[2]]
            ...
            support_label_padding_list.append(support_label_padding)
        ...
        if self.transform is not None:
            image, label, padding_mask = self.transform(image, label, padding_mask)   # transform the triple
            for k in range(self.shot):
                support_image_list[k], support_label_list[k], support_label_padding_list[k] = self.transform(support_image_list_ori[k], support_label_list_ori[k], support_label_padding_list[k])   # transform the triple
        ...
        if support_label_padding_list is not None:
            s_eys = support_label_padding_list
            s_ey = s_eys[0].unsqueeze(0)
            for i in range(1, self.shot):
                s_ey = torch.cat([s_eys[i].unsqueeze(0), s_ey], 0)
        # Return
        if self.mode == 'train':
            return image, label, s_x, s_y, subcls_list, padding_mask, s_ey
        elif self.mode == 'val':
            return image, label, s_x, s_y, subcls_list, raw_label, padding_mask, s_ey
        elif self.mode == 'demo':
...
class DatasetFSS(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        fss_list_root = './lists/fss-1000/{}/'.format(mode)
        fss_data_list_path = fss_list_root + 'data_list_{}.txt'.format(split)
        with open(fss_data_list_path, 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        support_label_list = [[] for _ in range(self.shot)]
        if self.transform is not None:
            query_img, query_mask = self.transform(query_img, query_mask)   # transform the triple
            for k in range(self.shot):
                support_image_list[k], support_label_list[k] = self.transform(support_imgs[k], support_masks[k])
        ...
        subcls_list.append(class_sample)

        # Return
        if self.mode == 'train':
            return query_img, query_mask, s_x, s_y, subcls_list
        elif self.mode == 'val':
            return query_img, query_mask, s_x, s_y, subcls_list, raw_label 
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_img = np.float32(query_img)
        query_id = query_name.split('/')[-1].split('.')[0]
        query_mask_name = os.path.join(os.path.dirname(query_name), query_id) + '.png'
        # 查询掩码
        # query_mask = self.read_mask(query_name)
        query_mask = cv2.imread(query_mask_name, cv2.IMREAD_GRAYSCALE)
        query_mask[query_mask < 128] = 0
        ...
        support_mask_names = [os.path.join(os.path.dirname(name), sid) + '.png' for name, sid in zip(support_names, support_ids)]
        for k in range(self.shot):  
            support_image_path = support_names[k]
            support_label_path = support_mask_names[k] 
            support_image = cv2.imread(support_image_path, cv2.IMREAD_COLOR)      
            support_image = cv2.cvtColor(support_image, cv2.COLOR_BGR2RGB)
            support_image = np.float32(support_image)
            support_label = cv2.imread(support_label_path, cv2.IMREAD_GRAYSCALE)
            ...
            # support_label[:,:] = 0
            # support_label[target_pix[0],target_pix[1]] = 1 
            support_label[support_label < 128] = 0
            support_label[support_label >= 128] = 1
        ...
        return query_img, query_mask, support_image_list_ori, support_label_list_ori
