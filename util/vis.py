...
class Visualizer:
    ...
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label, iou=None):
        ...
        spt_masked_pils = cls.merge_image_pair(spt_masked_pils)
        spt_masked_pils.save(cls.vis_path + '/s_%d' % (batch_idx) + '.jpg')
        qry_masked_pil.save(cls.vis_path + '/q_%d' % (batch_idx) + '.jpg')
        pred_masked_pil.save(cls.vis_path + '/pre_%d' % (batch_idx) + '.jpg')
