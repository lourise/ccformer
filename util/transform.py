...
class ToNumpy(object):
    ...
    def __call__(self, image, label, padding_mask=None):
        ...
        if padding_mask is not None:
            ...
            if not padding_mask.dtype == np.uint8:
                padding_mask = padding_mask.astype(np.uint8)
        return image, label, padding_mask
...
class Direct_Resize(object):
    ...
    def __call__(self, image, label, padding_mask=None):
        ...
        if padding_mask is not None:
            padding_mask = cv2.resize(padding_mask.astype(np.float32), dsize=(test_size, test_size),
                                      interpolation=cv2.INTER_NEAREST)
        return image, label, padding_mask
