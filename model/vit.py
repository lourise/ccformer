...
# from model import vit_utils as utils
...
class Block(nn.Module): # origin Block
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # self.attn = nn.MultiheadAttention(dim, num_heads=num_heads, bias=qkv_bias, dropout=0.1)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # Patch embedding
        self.patch_embed = embed_layer(  # embedsize=768=c*patchsize*patchsize
        ...
        self.blocks = nn.Sequential(*[
            Block(
                dim=embed_dim, num_heads=12, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
    ...
    def forward(self, x, prompt_token=None, mid_features_layer=None):
        # x: [B, C, H, W]
        if prompt_token is not None and prompt_token.size(1) == 2:
            bg_token, fg_token = prompt_token.split([1, 1], dim=1)
        elif prompt_token is not None and prompt_token.size(1) == 1:
            fg_token, bg_token = prompt_token, None
        else:
            fg_token, bg_token = None, None
        if mid_features_layer is None:
            mid_features_layer = []
        mid_features_list = []
        x = self.patch_embed(x)  # [B, N ,C] N: number of patches
        cls_token = expand_to_batch(self.cls_token, x.shape[0])  # [B, 1, C]
        if self.dist_token is None:
            x = cat_token(cls_token, x)     # [B, N+1, C]
        elif fg_token is None:
            dist_token = expand_to_batch(self.dist_token, x.shape[0])  # [B, 1, C]
            x = cat_token(cls_token, dist_token, x)     # [B, N+2, C]
        else:
            dist_token = expand_to_batch(self.dist_token, x.shape[0])  # [B, 1, C]
            x = cat_token(fg_token, dist_token, x)     # [B, N+2, C]
            # x = cat_token(prompt_token, x)     # [B, N+2, C]
        x = x + self.pos_embed  # [B, N + 1, C]
        if bg_token is not None:
            x = torch.cat([bg_token, x], dim=1)
            token_size = bg_token.size(1)
        else:
            token_size = 0
        x = self.pos_drop(x)
        ...
        for i, block in enumerate(self.blocks):
            x = block(x)
            if i in mid_features_layer:
                mid_features_list.append(x[:, token_size+self.num_tokens:, :].clone())

        x = self.norm(x)
        # print(x.shape)
        c_token = x[:, token_size, :].unsqueeze(1)

        x = x[:, token_size+self.num_tokens:, :]
        # print(x.shape)
        self.pv_cls_token = x[:, token_size, :].unsqueeze(1)
        b, n, c = x.shape
        hh = int(math.sqrt(n))
        x = x.view(b, hh, hh, c).permute(0, 3, 1, 2).contiguous()
        if mid_features_list is not None:
            return dict(out=x, cls_token=c_token, mid_features=mid_features_list)
        else:
            return dict(out=x, cls_token=c_token, mid_features=None)
...
def _load_weights_pth(logger, model: VisionTransformer, checkpoint_path):
    # if hasattr(model.patch_embed, 'model'):
...
vit_factory = {
    ...
    'ViT-B/16-dino':     {'patch_size': 16, 'embed_dim':  768, 'depth': 12, 'num_heads': 12, 'distilled': False},
