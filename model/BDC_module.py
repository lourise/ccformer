...
class BDC(nn.Module):
    ...
    def forward_hwc(self, x):
        if self.dr is not None and self.dr != self.input_dim:
            x = self.conv_dr_block(x)
        x = self.conv_dr_block(x)
        x = x.reshape(x.shape[0], x.shape[1], -1).transpose(1, 2)  # b, dim, dim
        x = BDCovpool(x, self.temperature)  # b, dim, dim
        # if self.is_vec:
        #     x = Triuvec(x)
        # else:
...
def BDCovpool(x, t):
    if len(x.data.shape) > 3:
        ...
        batchSize, dim, c = x.data.shape
        M = c
    # print('x.shape:', x.shape)
    I = torch.eye(dim, dim, device=x.device).view(1, dim, dim).repeat(batchSize, 1, 1).type(x.dtype)  # 4, 768, 768
    I_M = torch.ones(batchSize, dim, dim, device=x.device).type(x.dtype)  # 4, 768, 768
    x_pow2 = x.bmm(x.transpose(1, 2))  # 4, 768, 768
    dcov = I_M.bmm(x_pow2 * I) + (x_pow2 * I).bmm(I_M) - 2 * x_pow2  # eq6-1

    dcov = torch.clamp(dcov, min=0.0)
    dcov = torch.exp(t) * dcov
    dcov = torch.sqrt(dcov + 1e-5)  # eq6-2
    t = dcov - 1. / dim * dcov.bmm(I_M) - 1. / dim * I_M.bmm(dcov) + 1. / (dim * dim) * I_M.bmm(dcov).bmm(I_M)  # 4, 768, 768

    return t  # b, dim, dim; BDC matrix
...
def Triuvec(x):
    ...
    y = r[:, index].squeeze()  # 4, dim*(dim+1)/2
    return y