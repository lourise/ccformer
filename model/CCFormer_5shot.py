...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.b_norm = nn.BatchNorm2d(self.embed_dim)
        self.norm = nn.LayerNorm([self.embed_dim, 30, 30])
        self.norm1 = nn.LayerNorm([self.embed_dim, 60, 60])
        self.norm2 = nn.LayerNorm([self.embed_dim, 60, 60])
    ...
    def forward(self, x, s_x, s_y, y_m, chosen_cls=None, cat_idx=None, padding_mask=None, s_padding_mask=None, epoch=0):
        ...
        with torch.no_grad():
            for i in range(self.shot):
                ...
                supp_mask_out = self.original_encoder(s_x[:,i,:,:,:]*mask, mid_features_layer=self.mid_features_layer)   # [8, 768, 29, 29]
        ...
        for i in range(self.shot):
            ...
            sup_mask = F.interpolate(mask.view(B, 1, H, W), size=(h0, w0), mode='nearest')                                # [BS, 1, h0, w0]
            sup_mask_fg = (sup_mask == 1).float()                                               # [BS, 1, h0, w0]
            supp_feat_list.append(supp_feat)

        supp_feat = torch.cat(supp_feat_list, 0)    # [Bs, c, h, w]
        supp_mask = torch.cat(mask_list, 0)    # [Bs, 1, h, w]
        supp_feat = rearrange(supp_feat, '(b s) c h w -> b s c h w', b=B, s=S)
        supp_mask = rearrange(supp_mask, '(b s) 1 h w -> b s 1 h w', b=B, s=S)
        ...
        selected_query_feat_list = []
        selected_query_feat_hw_list = []
        for i in range(self.shot):
            supp_mask_i = supp_mask[:,i,:,:,:]
            supp_feat_i = supp_feat[:,i,:,:,:]
            query_final_feat, selected_query_feat, selected_query_feat_hw = self.bdc_metric_enhance(supp_feat_i, query_feat, supp_mask_i)
            selected_query_feat_list.append(selected_query_feat)
            selected_query_feat_hw_list.append(selected_query_feat_hw)
        selected_query_feat = torch.cat(selected_query_feat_list, 0)
        selected_query_feat_hw = torch.cat(selected_query_feat_hw_list, 0)
        selected_query_feat = rearrange(selected_query_feat, '(b s) c h w -> b s c h w', b=B, s=S)
        selected_query_feat_hw = rearrange(selected_query_feat_hw, '(b s) c h w -> b s c h w', b=B, s=S)
        query_final_feat = query_feat + selected_query_feat.mean(1) + selected_query_feat_hw.mean(1)
        query_final_feat = self.norm(query_final_feat)

        supp_feat = rearrange(supp_feat, 'b s c h w -> (b s) c h w')
        mask = rearrange(supp_mask, 'b s 1 h w -> (b s) 1 h w')
        supp_feat = self.purifier(supp_feat)
        query_feat = self.purifier(query_final_feat)

        sup_fts = rearrange(supp_feat.clone(), '(b s) c h w -> b s c h w', b=B, s=S)    # [B, s, c, h, w]
        ...
        supp_feat = rearrange(supp_feat, '(b s) c h w -> b s c h w', b=B, s=S)
        out_s = [self.classifier(qry_fts, supp_feat[:,i,...].unsqueeze(1), query_mask) for i in range(self.shot)]          # [B, 2, h, w]
        ...
        if self.training:
            ...
            aux_loss = self.criterion(out_s, mask.squeeze(1).long())
