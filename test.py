...
def get_parser():
    ...
    parser.add_argument('--dataset', type=str, default='pascal', choices=['pascal', 'coco', 'fss', 'DAVIS'])
    parser.add_argument('--night', type=bool, default=False, help='data root')
    parser.add_argument('--exp_id', type=int, default='540')
    parser.add_argument('--arch', type=str, default='CCFormer')
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0.yaml', help='config file') # pascal/pascal_split0_resnet50.yaml coco/coco_split0_resnet101.yaml
...
def get_val_path(args):
    val_path = args.result_path.replace("result", "val")
    return val_path
...
def get_model(args):
    ...
    if args.weight:
        ...
        if os.path.isfile(weight_path):
            ...
            # state_dict = {k: v for k, v in model_param.items() if 'original_encoder' not in k}
            new_param = checkpoint['state_dict']
            # new_param = checkpoint['model_state']
            # for item in model_param:
            #
            #     if item in new_param:
            #         state_dict[item] = new_param[item]
            ...
            try:
                ...
                # for key in list(model.parameters()):
                #     print(key)
            ...
            raise ValueError("=> no checkpoint found at '{}'".format(weight_path))
    # Get model para.
...
def main():
    ...
    logger = get_logger(args.snapshot_path)
    logger.info(model)
...
# ----------------------  DATASET  ----------------------
    ...
    # 可视化模块初始化
    val_path = get_val_path(args)
    args.val_path = val_path
    Visualizer.initialize(True, val_path)
    ...
    if args.evaluate:
        if args.resized_val:
            val_transform = transform.Compose([
                ...
                transform.Direct_Resize(size=args.val_size),  # 要用这个resize
                transform.ToTensor(),
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                       ...
                                       data_set=args.data_set, use_split_coco=args.use_split_coco, night=args.night)
    ...
    for val_id in range(val_num):
        ...
        loss_val, mIoU_val, mAcc_val, allAcc_val, class_miou, pIoU = validate(val_loader, model, val_seed, val_id) 
...
def validate(val_loader, model, val_seed, val_id):
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, cat_idx_list, ori_label, padding_mask, s_padding_mask) in enumerate(val_loader):
            ...
            if isinstance(input, list):    # multi-scale test
                ...
                for scale_id in range(scale_len):
                    ...
                    with torch.no_grad():
                        output_temp = model(s_x=s_input_temp, s_y=s_mask_temp, x=input_temp, y_m=target_temp, cat_idx=cat_idx_list)
                ...
                start_time = time.time()
                output = model(s_x=s_input, s_y=s_mask, x=input, y_m=target,
                                    cat_idx=cat_idx_list)
                ...
                output = output.max(1)[1]
                assert output.size() == target.size(), output.size()
                # 可视化
                # if (val_id == 0) and ((iter_num-1) % 10 == 0):
                # if (val_id == 0):
                #     Visualizer.visualize_prediction_batch(s_input, s_mask, input, target, output, cat_idx_list, iter_num-1, 0.0)
            # Metric
