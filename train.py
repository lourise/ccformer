...
def get_parser():
    ...
    parser.add_argument('--night', type=bool, default=False, help='query is night or not')
    parser.add_argument('--exp_id', type=int, default='540', help='experiment id')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0.yaml', help='config file') # coco/coco_split0.yaml  pascal/pascal_split3.yaml
...
# ----------------------  DATASET  ----------------------
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        ...
        train_data = dataset.DatasetFSS(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.train_list, \
                                    transform=train_transform, mode='train', \
    ...
    if args.evaluate:
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            ...
            val_data = dataset.DatasetFSS(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.val_list, \
                                    transform=val_transform, mode='val', \
                                    data_set=args.data_set, use_split_coco=args.use_split_coco)                              
...
# ----------------------  TRAINVAL  ----------------------
    ...
    global best_miou_m, best_FBiou_m
    best_miou = 0.
    best_FBiou = 0.
    best_piou = 0.
    best_epoch = 0
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, s_input, s_mask, subcls, padding_mask, s_padding_mask) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, cat_idx=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask)
        ...
        # save model for <testing>
            if mIoU > best_miou:
                if previous_best_pth is not None:
                    os.remove(previous_best_pth)
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, subcls, ori_label, padding_mask, s_padding_mask) in enumerate(val_loader):
            ...
            start_time = time.time()
            with torch.no_grad():
                output = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, cat_idx=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask)
